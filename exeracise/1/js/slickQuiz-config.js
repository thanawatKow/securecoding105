...
var quizJSON = {
    "info": {
        "name":    "เเบบทดสอบ",
        "main":    "<p>เเบบทดสอบจะประกอบไปด้วยเเบบทดสอบทั้งหมด 20 ข้อด้วยกัน ซึ่งคำถามจะประกอบไปด้วยเนื้อหาเเต่ละบทรวมกัน </p>",
        "results": "",
        "level1":  "ยอดเยี่ยม",
        "level2":  "ดีมาก",
        "level3":  "ปานกลาง",
    ...
    "questions": [
        { // Question 1 
            "q":"1.ฟังก์ชันนี่เป็นส่วนหนึ่งของโปรแกรมที่รันบนระบบ 32 x86 bit ซึ่งคอมไพเลอร์ไม่มีการเปลี่ยนค่าลำดับของตัวแปรบนstack<p>void function(char *input)</p><p>{</p><p>&nbsp;&nbsp;int i = 1;</p><p>&nbsp;&nbsp;char buffer[8];</p><p>&nbsp;&nbsp;int j = 2;</p><p>&nbsp;&nbsp;strcpy(buffer,input);</p><p>&nbsp;&nbsp;printf(%x %x %s \n, i,j,buffer);</p><p>}</p>อะไรคือค่าที่น้อยสุดของความยาวของสตริงที่ส่งผ่านอินพุตของพารามิเตอร์ที่ทำให้โปรแกรมเกิดการแคลช",
            "a": [
            ...
            "incorrect": "<p><span>ผิด ครับ</span> คำตอบคือ 12 ครับ เพราะ ตั้งแต่ สตริงมีการเก็บค่าและจนกระทั่งล้นไปได้ถึง 13 ไบต์ และ สตริงที่จะทำให้เกิดการแคลชของโปรแกรมคือตัวแรกที่เขียนทับ EBP</p>" // no comma here
        },
        ...
        { // Question 4
            ...
            "a": [
                ...
                {"option": "3.Buffer overflows สามารถใช้ปรับเปลี่ยน state และ operation ของ ช่องโหว่ Application ",      "correct": true},
		...
		{// Question 6
            ...
            "correct": "<p><span>ถูกต้อง</span>เก่งมากๆๆ เลย ครับ </p>",
            "incorrect": "<p><span>ผิด ครับ </span>เข้อ 1.char pwd[37], n[ll+1]; ไมใช่ เพราะ่ นี้เป็นการแก้ไข off-by-one error  ใน strncpy  แต่ไม่สามารถแก้ไขช่องโหว่ printf <p>ข้อ 2.#define ll 13 ไม่ใช่ เพราะ ไม่มีการแก้ไข bug มีแต่การเพิ่มขนาด size <p>ข้อ 3.void s(char *u) {strncpy(n,u,ll-1); printf(n);}  ไม่ใช่ เพราะ  นี้เป็นการแก้ไข off-by-one error  ใน strncpy  แต่ไม่สามารถแก้ไขช่องโหว่ printf</p><p>ข้อ 4. void s(char *u) {strncpy(n,u,11); printf(“%s”, n);}ใช่ เพราะ มีการแก้ไขทั้ง off-by-one error   และ ช่องโหว่ printf แต่ยากที่เข้ารหัสจำนวนตัวอักษรที่จะคัดลอกอาจก่อให้เกิดปัญหาในอนาคต </p><p>ข้อ 5. void s(char *u) {strncpy(n,u,ll-1); cout << n;} ใช่ เพราะ  มีการแก้ไขทั้ง off-by-one error   และ ช่องโหว่ printf </p></p>" // no comma here
        } ,// no comma here
		{// Question 7
		...
		{// Question 8
			"q": "ถ้าในโค้ดด้านล่าง toprint points ที่ '% 08X% 08X% 08X% 08X% 08X% N' อะไรคือ character ที่ผู้ใช้ต้องใส่เพื่อที่จะผ่านการตรวจสอบรหัสผ่านโดยไม่คำนึงถึงค่าของรหัสผ่านที่อ่านได้จาก SecretPassword.txt?<p>#define INPUT_SIZE  16</p><p> bool checkPass(char *toprint)</p><p>{</p><p>&nbsp;&nbsp;bool result;</p><p>&nbsp;&nbsp;char input[INPUT_SIZE]=' ';</p><p>&nbsp;&nbsp;char  *secret;</p><p>&nbsp;&nbsp;secret=(char*)malloc(INPUT_SIZE);</p><p>&nbsp;&nbsp;Readfile(SecretPassword.txt,secret,INPUT_SIZE);</p><p>&nbsp;&nbsp;printf(toprint);</p><p>&nbsp;&nbsp;gets_s(input,INPUT_SIZE);</p><p>&nbsp;&nbsp;result=!strcmp(secret,input);</p><p>&nbsp;&nbsp;free(secret);</p><p>&nbsp;&nbsp;return result;</p><p>}</p>",
            "a": [
		...
		{// Question 9
            ...
            "correct": "<p><span>ถูกต้อง</span>เก่งมากๆๆ เลย ครับ </p>",
            "incorrect": "<p><span>ผิด ครับ</span>คำตอบที่ถูกคือ ข้อ 1. การเปลี่ยนค่า return address ให้กลายเป็นฟังก์ชัน system พร้อมกับระบุค่าอาร์กิวเมนต์ของฟังก์ชันให้เป็นบัฟเฟอร์ที่ใส่เข้าไป ทำให้แฮกเกอร์สามารถเรียกคำสั่งอะไรก็ได้</p>" // no comma here
        },
        {// no // Question 10
		"q": "การใช้ฟังก์ชัน ฟังก์ชัน strcpy มีความปลอดภัยหรือไม่ เเละ ถ้าไม่ปลอดภัยเราสามารถใช้อะไรเพื่อลดความเสี่ยงที่เกิดขึ้นได้ ",
            "a": [
                ...
                {"option": "2.ไม่ปลอดภัย เพราะ ไม่ได้ตรวจสอบการเกิด buffer overflow ขณะที่ทำสำเนาไปยังเป้าหมาย สามารถใช้  strcpy_s ",     "correct": true},
		...
		"q": "จากโปรเเกรมต่อไปนี้ ค่าของ i ทั้งสองค่ามีค่าเท่าไร เเละ เพราะ อะไรถึงเป็นเช่นนั้น<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;int unsigned short int u = 32768;</p><p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;int short int i;</p><p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;int i = u;</p><p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;int printf(i = %d\n, i);</p><p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;int u = 65535;</p><p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;int i = u;</p><p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;int printf(i = %d\n, i); </p>",
            "a": [
                ...
                {"option": "2.i=32768  i= 1 เพราะ เกิดจาก Integer Overflow ",     "correct": false},
		...
		"q": "ในโค้ดที่เเสดงนี้สามารถทำเกิด buffer overflow เเละ sign error ได้ เราสามารถเเก้โค้ดได้อย่างไรบ้าง เพื่อเป็นการตรวจสอบเเละป้องกันการเกิด overflow ได้ <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;1.#define BUFF_SIZE 10</p><p>2.int main(int argc, char* argv[]){ </p> <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;3.int len; </p> <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.char buf[BUFF_SIZE];</p><p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;5.len = atoi(argv[1]); </p><p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;6.if (len < BUFF_SIZE){</p><p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;7.memcpy(buf, argv[2], len);</p><p>8.}</p><p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;9.else</p><p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;10.printf(Too much data\n);</p>  ",
            "a": [
				...
				{"option": "3.เเก้ไข บรรทัดที่ 6. if((0 < len) && (len < BUFF_SIZE) ) เเละ เเก้ไข บรรทัดที่ 3 unsigned int len ;" ,    "correct": true},  
				{"option": "4.เเก้ไข บรรทัดที่ 6. if((0 >len) && (len < BUFF_SIZE) ) เเละ เเก้ไข บรรทัดที่ 3 unsigned int len ;",    "correct": false},
            ],
            "correct": "<p><span>ถูกต้อง</span>เก่งมากๆๆ เลย ครับ </p>",
            "incorrect":"<p><span>ผิด ครับ</span>คำตอบที่ถูกต้องคือ ข้อที่ 3 เเก้ไขบรรทัดที่ 6  0<len && len < BUFF_SIZE จะช่วยตรวจสอบขนาดของ array เพื่อป้องการเกิด overflow ได้ เเละ เเก้ไขบรรทัดที่ 3 unsigned int len เพื่อป้องการเกิดเงื่อนไขที่เกิดจากตัวที่เป็น signed ได้</p>"
        }, 
		{ // Question 18
